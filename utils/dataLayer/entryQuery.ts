...
export const getStaticPropsWrapper = (
  incomingQuery: string | DocumentNode,
  variables: (
    context: GetStaticPropsContext<any, StatamicPreviewData>,
  ) => any,
) => async (
  ...
  // If we find a query that was wrapped in gql, make it a string for the request.
  const query = typeof incomingQuery === 'string'
    ? incomingQuery
    : convertGqlToString(incomingQuery)
